...
const DocumentSidebar: React.FC<DocumentSidebarProps> = ({ documents }) => {
  ...
  return (
  <div className={cn('flex flex-col h-full bg-white text-black border-l p-4 gap-4 transition-all duration-200', collapsed ? 'w-14' : 'w-80')}>
      <div className="flex items-center justify-between">
        <CardHeader className={cn('p-2 pt-0', collapsed && 'hidden')}>
          <CardTitle className="text-lg flex items-center">
            <Paperclip className="mr-2 h-5 w-5 text-white" />
            Attached Documents
          </CardTitle>
        </CardHeader>
        <button
            ...
            <ul className="space-y-3">
              {documents.map((doc) => (
                <li key={doc.id} className="flex items-center text-sm text-muted-foreground p-2 rounded-md hover:bg-muted/50">
                  <FileText className="h-4 w-4 mr-3 flex-shrink-0 text-white" />
